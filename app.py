...
st.set_page_config(page_title="Music Opportunity Finder V2", page_icon="🎵", layout="wide", initial_sidebar_state="expanded")
DB = "music_finder_v2.db"
...
def relevance_score(row, song="", artist=""):
    """Score YouTube candidates against an exact song + artist pair."""
    title=normalize(row.get("title","")); ch=normalize(row.get("channel","")); ar=normalize(row.get("artist",""))
    ns=normalize(song); na=normalize(artist)
    score=0
    if ns:
        if ns == title: score += 100
        elif ns in title: score += 70
        else:
            overlap=len(set(ns.split()) & set(title.split()))
            score += min(40, overlap*10)
    if na:
        if na == ar: score += 100
        elif na in ar: score += 80
        elif na in ch: score += 65
        elif na in title: score += 55
    if ns and na and ns in title and (na in title or na in ch or na in ar): score += 100
    if NOISE.search(row.get("title","")): score -= 100
    return score
...
@st.cache_data(ttl=180, show_spinner=False)
def multi_search(keyword, limit, song="", artist=""):
    """Fast multi-query search. When song+artist are supplied, prioritize exact pair matches."""
    keyword=(keyword or "").strip(); song=(song or "").strip(); artist=(artist or "").strip()
    if song and artist:
        qs=[f'"{song}" "{artist}"', f'{song} {artist} official', f'{song} {artist} song', f'{song} - {artist}']
    else:
        qs=[keyword, f"{keyword} official song", f"{keyword} music video"]
    per=max(5,min(30,int(limit//2)+4))
    merged=[]; seen=set()
    with ThreadPoolExecutor(max_workers=min(4,len(qs))) as ex:
    ...
    if song and artist:
        merged.sort(key=lambda r:(relevance_score(r,song,artist), int(r.get("views",0))), reverse=True)
        # Keep strong pair matches first; allow fallback candidates after them.
        strong=[r for r in merged if relevance_score(r,song,artist)>=90]
        rest=[r for r in merged if r not in strong]
        return (strong+rest)[:int(limit)]
...
with st.sidebar:
    ...
    song_name=""; artist_name_query=""
    if mode=="🔥 Song Discovery":
        song_name=st.text_input("🎵 Exact Song Name (optional)", "")
        artist_name_query=st.text_input("🎤 Singer / Artist Name (optional)", "")
        st.caption("For the most accurate result, enter BOTH Song Name + Singer/Artist Name.")
    else:
...
if st.button("🚀 FAST FIND & RESEARCH SONGS",type="primary",use_container_width=True):
    with st.spinner("⚡ Fast multi-search…"):
        search_term = (f'"{song_name}" "{artist_name_query}"' if song_name.strip() and artist_name_query.strip() else (song_name.strip() or keyword))
        rows=multi_search(search_term,int(results),song_name,artist_name_query)
    if song_name.strip() and artist_name_query.strip():
        rows=sorted(rows,key=lambda r:(relevance_score(r,song_name,artist_name_query), int(r.get("views",0))),reverse=True)
        strong=[r for r in rows if relevance_score(r,song_name,artist_name_query)>=90]
        rows = strong[:int(results)] if strong else rows[:int(results)]
    rows=filter_rows(rows,int(min_views),int(max_views),int(year),hide_noise)
    if not rows: st.warning("No candidates. Try the exact Song Name + Singer Name, Any Year / 0, or lower Minimum views."); st.stop()
